<div class="col-sm-6 col-md-4">
    <div class="thumbnail" style="border:solid 1.5px forestgreen; background-color: beige">
        <div class="labels">
...
{{--        <img src="{{ Storage::url($sku->product->image) }}" alt="{{ $sku->product->__('name') }}">--}}
        <img src="{{asset('storage/'.$sku->product->image)}}" alt="{{ $sku->product->__('name') }}">
        <div class="caption">
            <h3>{{ $sku->product->__('name') }}</h3>
{{--            @isset($sku->product->properties)--}}
{{--                @foreach ($sku->propertyOptions as $propertyOption)--}}
{{--                    <h4>{{ $propertyOption->property->__('name') }}: {{ $propertyOption->__('name') }}</h4>--}}
{{--                    <h4>{{ $propertyOption->__('name') }}</h4>--}}
{{--                @endforeach--}}
{{--            @endisset--}}
            <h4>{{$sku->product->author}}</h4>
            <h4>{{$sku->product->genres}}</h4>
            <h4>{{$sku->product->year}}</h4>
